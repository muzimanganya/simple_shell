 ...
 * Ifeoma Onyealisi and Desire Habimana
 * for Alx School
 * Return: 0 if success
 */
int main(void)
{
	...
	char *entry = NULL, *arguments[20]; /**String of args entered by the usr*/
	...
	while (bytes_rd != -1)
	{
		if (*entry != '\n')
		{
			...
			if (arguments[0] != NULL)
			{
				exist_stat = exist(arguments[0]);/**Exist evaluates if entered path exists*/
